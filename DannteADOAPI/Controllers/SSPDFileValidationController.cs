...
namespace DannteADOAPI.Controllers
{
    ...
    [ApiController]
    public class SSPDFileValidationController : Controller
    {
        readonly ISSPDFileValidationServices SSPDServices;
        private readonly IHubContext<NotificationHub> _hubContext;

        public SSPDFileValidationController(ISSPDFileValidationServices _sspdServices, IHubContext<NotificationHub> hubContext)
        {
        ...
        /// <returns></returns>  
        [HttpPost]
        [Route(nameof(SSPDFileValidationController.ReadFilesSspd))]
        public async Task<IActionResult> ReadFilesSspd(LacValidationDTO request)
        {
            ResponseQuery<bool> response = new ResponseQuery<bool>();
            await AddMessage(true, "SSPD se está Validando");
